...
pcall(function()
	if type(isfile) == "function"
		...
		if type(loaded) == "table" then
			...
			if type(loaded.theme) == "string" then
...
local THEMES = {
	...
	Grey = {
		Background = "rbxassetid://121965938312103",
	...
	Red = {
		Background = "rbxassetid://74882657109121",

		Window = Color3.fromRGB(15, 5, 6),
		Row = Color3.fromRGB(30, 10, 12),
		Control = Color3.fromRGB(55, 18, 21),
		Log = Color3.fromRGB(12, 5, 6),

		White = Color3.fromRGB(255, 240, 240),
		Text = Color3.fromRGB(220, 185, 185),
		Dim = Color3.fromRGB(145, 105, 105),

		Accent = Color3.fromRGB(220, 70, 70)
	},

	Green = {
		Background = "rbxassetid://82608438134992",

		Window = Color3.fromRGB(5, 14, 8),
		Row = Color3.fromRGB(10, 28, 16),
		Control = Color3.fromRGB(18, 52, 30),
		Log = Color3.fromRGB(5, 12, 7),

		White = Color3.fromRGB(235, 255, 240),
		Text = Color3.fromRGB(180, 220, 190),
		Dim = Color3.fromRGB(105, 145, 115),

		Accent = Color3.fromRGB(75, 205, 110)
	},

	Yellow = {
		Background = "rbxassetid://129439754269179",

		Window = Color3.fromRGB(15, 13, 5),
		Row = Color3.fromRGB(30, 27, 10),
		Control = Color3.fromRGB(55, 48, 16),
		Log = Color3.fromRGB(12, 11, 5),

		White = Color3.fromRGB(255, 253, 230),
		Text = Color3.fromRGB(220, 215, 175),
		Dim = Color3.fromRGB(145, 140, 100),

		Accent = Color3.fromRGB(225, 195, 55)
	}
}
...
local function loadTheme(themeName)
	local theme = THEMES[themeName] or THEMES.Grey
...
local oldGui =
	CoreGui:FindFirstChild("ShadowAutoRNG")
	or playerGui:FindFirstChild("ShadowAutoRNG")
...
BackgroundImage.Image = COLORS.Background
...
local function makeTab(text, position, width)
...
local WhitelistTabButton =
	makeTab(
		"WHITELIST",
		UDim2.new(0.333, 1, 0, 2),
		0.333
	)

local ThemesTabButton =
	makeTab(
		"THEMES",
		UDim2.new(0.666, 1, 0, 2),
		0.334
	)
...
ThemesPage.ScrollBarThickness = 3

ThemesPage.ScrollBarImageColor3 =
	COLORS.Dim

ThemesPage.AutomaticCanvasSize =
	Enum.AutomaticSize.Y

ThemesPage.CanvasSize =
	UDim2.new(0, 0, 0, 0)

ThemesPage.Visible = false

ThemesPage.ZIndex = 3
ThemesPage.Parent = Window
...
local function applyTheme(themeName)
	...
	COLORS.Window = theme.Window
	COLORS.Row = theme.Row
	COLORS.Control = theme.Control
	COLORS.Log = theme.Log

	COLORS.White = theme.White
	COLORS.Text = theme.Text
	COLORS.Dim = theme.Dim
...
local function makeThemeButton(
	themeName,
	themeData
)
	local button = Instance.new("TextButton")
	...
	local backgroundText =
		makeLabel(
			...
			themeName == "Grey"
				...
				or "BACKGROUND",
			UDim2.new(1, -70, 0, 14),
			UDim2.fromOffset(24, 23),
			7,
			themeData.Text,
			Enum.Font.GothamMedium
		)

	backgroundText.ZIndex = 5
...
local function setTab(name)
	...
	ThemesTabButton.BackgroundColor3 =
		themes
		and COLORS.White
		or COLORS.Control

	ThemesTabButton.TextColor3 =
		themes
		and COLORS.Window
		or COLORS.Dim
	...
	if main then
		targetHeight = MAIN_HEIGHT
	elseif whitelist then
		targetHeight = WHITELIST_HEIGHT
	else
		targetHeight = THEMES_HEIGHT
	end
...
ThemesTabButton.Activated:Connect(function()
	setTab("THEMES")
end)
...
local function promptAttachmentPosition(
	attachment
)
